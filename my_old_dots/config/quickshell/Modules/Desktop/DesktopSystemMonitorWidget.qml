...
PanelWindow {
    ...
    readonly property string nvmlScriptPath: Paths.strip(Qt.resolvedUrl("../../scripts/nvidia_gpu_temp.py"))
    readonly property var nvmlCommand: ["python3", nvmlScriptPath]
    readonly property bool nvmlSupported: SettingsData.desktopGpuSelection && SettingsData.desktopGpuSelection.toLowerCase().indexOf("nvidia") !== -1
    ...
    readonly property bool showGpuTemperature: currentGpuTemperature > 0 && (nvmlSupported || currentGpuTemperature < 90)
                    ...
                    Column {
                        ...
                        Item {
                            ...
                            Column {
                                ...
                                StyledText {
                                    text: showGpuTemperature ? Math.round(currentGpuTemperature) + "°C" : "--°C"
                                    font.pixelSize: 20 * scaleFactor
                                    font.weight: Font.Bold
                                    color: {
                                        if (!showGpuTemperature) return Theme.surfaceTextMedium
                            ...
                            Column {
                                ...
                                StyledText {
                                    text: {
                                        if (showGpuTemperature && currentGpuMemoryUsed > 0) {
                                            const usedGB = (currentGpuMemoryUsed / 1024).toFixed(1)
                                            return usedGB + "GB"
                                        }
                                        return showGpuTemperature ? "Active" : "Offline"
                                    }
                                    font.pixelSize: 20 * scaleFactor
                                    font.weight: Font.Bold
                                    color: showGpuTemperature ? Theme.surfaceText : Theme.surfaceTextMedium
                                    anchors.right: parent.right
                                }
                            }
                        }
