...
Singleton {
    ...
    property bool nvmlEnabled: Qt.application && Qt.application.arguments ? Qt.application.arguments.indexOf("--enable-nvml") !== -1 : false
    ...
    Process {
        id: nvmlCheckProcess
        command: ["python3", "-c", "import pynvml; print('NVML available')"]
        running: false
        onExited: exitCode => {
    ...
    Process {
        id: nvmlGpuProcess
        command: ["python3", nvmlScriptPath]
        running: false
        onExited: exitCode => {
            if (exitCode !== 0) {
