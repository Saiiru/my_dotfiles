...
Singleton {
    ...
    readonly property string quickshellDir: Paths.expandTilde("~/.config/quickshell")
    readonly property string gpuTelemetryScript: quickshellDir + "/scripts/nvidia_gpu_temp.py"
    ...
    Process {
        id: nvmlGpuProcess
        command: ["python3", gpuTelemetryScript]
        running: false
        onExited: exitCode => {
            if (exitCode !== 0) {
