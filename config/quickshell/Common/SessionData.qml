...
Singleton {
    ...
    FileView {
        ...
        path: StandardPaths.writableLocation(StandardPaths.GenericStateLocation) + "/quickshell/session.json"
        blockLoading: true
        blockWrites: true
        watchChanges: true
        onLoaded: {
    ...
    Process {
        ...
        command: ["sh", "-c", "CONFIG_DIR=\"" + _stateDir
            + "/quickshell\"; if [ -f \"$CONFIG_DIR/default-session.json\" ] && [ ! -f \"$CONFIG_DIR/session.json\" ]; then cp \"$CONFIG_DIR/default-session.json\" \"$CONFIG_DIR/session.json\" && echo 'copied'; else echo 'not_found'; fi"]
        running: false
        onExited: exitCode => {
            if (exitCode === 0) {
