...
notif_icon="$HOME/.config/swaync/images/ja.png" # Assuming this is the correct path for a generic icon

# --- Dependency Check ---
check_dependency() {
    command -v "$1" >/dev/null 2>&1 || { notify-send -i "$notif_icon" "Error: Missing Dependency" "$1 is not installed. Aborting."; exit 1; }
}
...
if [ -f "$settings_file" ]; then
  ...
  notify-send -i "$notif_icon" "Error: Config File" "Settings file not found: $settings_file"
  exit 1
fi
...
if ! change_layout; then
    notify-send -u low -t 2000 -i "$notif_icon" 'Keyboard Layout' " Error: Layout change failed"
    echo "Layout change failed." >&2
    exit 1
else
    notify-send -u low -i "$notif_icon" " Keyboard Layout:" " $new_layout"
