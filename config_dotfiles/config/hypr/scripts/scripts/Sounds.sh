...
# --- Dependency Check ---
check_audio_player() {
    if command -v pw-play >/dev/null 2>&1 || command -v pa-play >/dev/null 2>&1; then
        return 0 # At least one player is available
    else
        echo >&2 "Error: Neither 'pw-play' (PipeWire) nor 'pa-play' (PulseAudio) is installed. Aborting sound playback."
        exit 1
    fi
}
