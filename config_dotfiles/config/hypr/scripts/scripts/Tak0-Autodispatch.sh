...
# --- Configuration ---
MAX_RETRIES=30      # Number of times to check for the window
RETRY_INTERVAL=0.3  # Seconds to wait between checks
...
if [[ -z "$APP" || -z "$TARGET_WORKSPACE" ]]; then
    ...
    notify-send -i "$iDIR/error.png" "Autodispatch Error" "Usage: $0 <application_command> <target_workspace_number>"
    exit 1
fi
...
for i in $(seq 1 "$MAX_RETRIES"); do
    ...
    if [[ -n "$win" ]]; then
        ...
        notify-send -i "$iDIR/ja.png" "Autodispatch" "'$APP' dispatched to workspace $TARGET_WORKSPACE"
        exit 0
    fi
...
notify-send -i "$iDIR/error.png" "Autodispatch Error" "Window for '$APP' was NOT found or dispatched properly to workspace $TARGET_WORKSPACE"
