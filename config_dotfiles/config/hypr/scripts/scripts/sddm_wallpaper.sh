...
check_dependency "$terminal" # Check for the terminal itself
...
# Check if rofi_wallust exists
if [[ ! -f "$rofi_wallust" ]]; then
    notify-send -i "$iDIR/error.png" "Error: Missing File" "Rofi wallust config not found: $rofi_wallust. Aborting."
    exit 1
fi
...
if [[ ! -f "$wallpaper_path" ]]; then
    notify-send -i "$iDIR/error.png" "Error: Missing File" "Selected wallpaper not found: $wallpaper_path. Aborting."
    exit 1
fi
