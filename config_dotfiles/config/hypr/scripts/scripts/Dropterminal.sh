...
if [ -z "$TERMINAL_CMD" ]; then
  notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Missing terminal command. Usage: $0 [-d] <terminal_command>"
...
get_monitor_info() {
  ...
  if [ -z "$monitor_data" ] || [[ "$monitor_data" =~ ^null ]]; then
    ...
    notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Could not get focused monitor information."
    return 1
  fi
  echo "$monitor_data"
}
...
calculate_dropdown_position() {
  ...
  if [ $? -ne 0 ] || [ -z "$monitor_info" ]; then
    ...
    notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to get monitor info, using fallback values."
...
spawn_terminal() {
  ...
  if [ $? -ne 0 ]; then
    ...
    notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to calculate position, using fallback."
  fi
  ...
  hyprctl dispatch exec "[float; size $width $height; workspace special:scratchpad silent] $TERMINAL_CMD" || { notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to execute terminal command: $TERMINAL_CMD"; return 1; }
  ...
  notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to get terminal address after spawning."
  return 1
}
...
if terminal_exists; then
  ...
  focused_monitor=$(get_monitor_info | cut -d' ' -f6) # Only need monitor name here
  ...
  if [ "$focused_monitor" != "$dropdown_monitor" ]; then
    ...
    if [ $? -ne 0 ]; then
        notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to calculate new position for monitor change."
        exit 1
    fi
  ...
  if terminal_in_special; then
    ...
    pos_info=$(calculate_dropdown_position)
    if [ $? -ne 0 ]; then
        notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to calculate position for showing terminal."
        exit 1
    fi
    target_x=$(echo "$pos_info" | cut -d' ' -f1)
    target_y=$(echo "$pos_info" | cut -d' ' -f2)
    ...
    if [ -n "$geometry" ]; then
      ...
      notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Could not get window geometry, moving to scratchpad without animation."
      hyprctl dispatch pin "address:$TERMINAL_ADDR"
      hyprctl dispatch movetoworkspacesilent "$SPECIAL_WS,address:$TERMINAL_ADDR"
    fi
  fi
  ...
  if spawn_terminal; then
    ...
    if [ -n "$TERMINAL_ADDR" ]; then
      ...
      notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to focus newly spawned terminal."
    fi
  else
    notify-send -i "$iDIR/error.png" "Error: Dropdown Terminal" "Failed to spawn new terminal."
  fi
fi
