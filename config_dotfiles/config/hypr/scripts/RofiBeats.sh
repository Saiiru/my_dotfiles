...
populate_local_music() {
  if [[ ! -d "$mDIR" ]]; then
    notification "Error" "Music directory not found: $mDIR"
    return 1
  fi
  ...
  done < <(find -L "$mDIR" -type f \( -iname "*.mp3" -o -iname "*.flac" -o -iname "*.wav" -o -iname "*.ogg" -o -iname "*.mp4" \) 2>/dev/null)

  if [ ${#local_music[@]} -eq 0 ]; then
    notification "No Local Music" "No supported music files found in $mDIR"
    return 1
  fi
  return 0
}
...
play_local_music() {
  ...
  for ((i = 0; i < "${#filenames[@]}"; ++i)); do
    if [ "${filenames[$i]}" = "$choice" ]; then
      ...
      mpv --no-video --playlist-start="$i" --loop-playlist "${local_music[@]}" || notification "Error" "Failed to play $choice"
      break
    fi
  done
}
...
shuffle_local_music() {
  ...
  # Check if there are any music files to shuffle
  if ! find -L "$mDIR" -type f \( -iname "*.mp3" -o -iname "*.flac" -o -iname "*.wav" -o -iname "*.ogg" -o -iname "*.mp4" \) -print -quit | grep -q .; then
    notification "No Local Music" "No supported music files found in $mDIR to shuffle."
    return 1
  fi

  music_playing && stop_music
  notification "Shuffle Play local music"
  mpv --no-video --shuffle --loop-playlist "$mDIR" || notification "Error" "Failed to shuffle play local music"
}
...
play_online_music() {
  ...
  music_playing && stop_music
  notification "Now Playing:" "$choice"
  mpv --no-video --shuffle "$link" || notification "Error" "Failed to play online stream: $choice"
}
