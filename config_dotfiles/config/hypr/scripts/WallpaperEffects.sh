...
check_dependency "magick" # ImageMagick
check_dependency "swww"
check_dependency "wallust"
check_dependency "notify-send"
check_dependency "hyprctl"
check_dependency "jq"
check_dependency "yad" # Used by sddm_wallpaper.sh, which is called by this script
...
if [[ ! -f "$wallpaper_current" ]]; then
  notify-send -i "$iDIR/error.png" "E-R-R-O-R" "Current wallpaper file not found: $wallpaper_current"
  exit 1
fi
